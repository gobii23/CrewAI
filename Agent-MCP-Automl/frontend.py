...
st.set_page_config(
    page_title="My Own Agent", layout="wide", initial_sidebar_state="expanded"
)
...
def show_enhanced_results(folder, report_name, section_name):
    ...
    with col2:
        ...
        if os.path.exists(folder):
            ...
            last_modified = datetime.fromtimestamp(folder_mtime).strftime(
                "%Y-%m-%d %H:%M"
            )
    ...
    if img_files:
        ...
        for i, img in enumerate(img_files):
            with cols[i % len(cols)]:
                ...
                st.image(
                    img_path, caption=img.replace("_", " ").title(), width="stretch"
                )
...
with st.sidebar:
    ...
    if st.session_state["history"]:
        for i, pipeline_id in enumerate(st.session_state["history"][:5]):
            pipeline_info = st.session_state.get("pipeline_configs", {}).get(
                pipeline_id, {}
            )
...
if step == 1:
    ...
    with col1:
        ...
        if uploaded_file:
            ...
            if st.button("Next: Configure Pipeline", type="primary", width="stretch"):
    ...
    with col2:
        ...
        if recent_uploads:
            for upload in recent_uploads[:5]:
                if st.button(
                    f"{upload['original_name'][:20]}...\n{upload['timestamp']}",
                    key=f"recent_{upload['timestamp']}",
                ):
...
# Step 2: Configuration
    ...
    with col1:
        ...
        selected_objective = st.radio(
            "Choose your ML objective:", objective_options, horizontal=True
        )
        ...
        if df is not None:
            ...
            target_column = st.selectbox(
                ...
                index=(
                    df.columns.tolist().index(suggested_targets[0])
            ...
            target_column = st.text_input(
                "Target Column",
                value="label",
                help="Enter the name of your target column",
            )
        ...
        with st.expander("Preprocessing Options", expanded=False):
            handle_missing = st.selectbox(
                "Handle Missing Values", ["auto", "drop", "impute"]
            )
    ...
    with col2:
        ...
        config_valid = all(
            [st.session_state.get("objective"), st.session_state.get("target_column")]
        )
    ...
    with col1:
        if st.button("Back to Upload", width="stretch"):
            ...
            st.rerun()
    with col3:
        if config_valid and st.button(
            "Next: Execute Pipeline", type="primary", width="stretch"
        ):
...
# Step 3: Execution
    ...
    with col1:
            ...
            with st.spinner("Launching full pipeline..."):
                try:
                    response = requests.post(
                        f"{API_BASE}/run/automl/full", json=payload
                    )
                    if response.status_code == 200:
                        result = response.json()
                        st.session_state["pipeline_id"] = result["pipeline_id"]
                        st.session_state["dataset_name"] = st.session_state[
                            "file_path"
                        ].replace(".csv", "")
        ...
        if st.button("Run EDA Only", width="stretch"):
            payload = {
                "file_path": st.session_state["file_path"],
                "objective": st.session_state["objective"],
            }
    ...
    with col2:
        ...
        if st.button("Run Preprocessing", width="stretch"):
            ...
            if "preprocessing_config" in st.session_state:
                payload["preprocessing_config"] = st.session_state[
            ...
            with st.spinner("Processing your data..."):
                try:
                    response = requests.post(
                        f"{API_BASE}/run/automl/preprocessing", json=payload
                    )
                    if response.status_code == 200:
                        st.session_state["dataset_name"] = st.session_state[
                            "file_path"
        ...
        if st.button("Run Training Only", width="stretch"):
            payload = {
                "file_path": st.session_state["file_path"],
                "objective": st.session_state["objective"],
            ...
            with st.spinner("Training models..."):
                try:
                    response = requests.post(
                        f"{API_BASE}/run/automl/training", json=payload
                    )
                    if response.status_code == 200:
                        st.session_state["dataset_name"] = st.session_state[
                            "file_path"
...
# Step 4: Results and Monitoring
                ...
                try:
                    ...
                    if status_response.status_code == 200:
                        ...
                        with progress_placeholder.container():
                            ...
                            st.progress(
                                progress_value,
                                text=f"Pipeline Progress: {progress_value}%",
                            )
                        ...
                        st.warning(
                            f"Unable to fetch status (HTTP {status_response.status_code})"
                        )
                        break
    ...
    with tab_compare:
        ...
        comparison_file = (
            f"{base_dir}/{dataset_name}_final_evaluation/model_comparison.json"
        )
    ...
    with action_col1:
        if st.button("Run New Pipeline", type="primary", width="stretch"):
            ...
            keys_to_remove = [
                k for k in st.session_state.keys() if k not in keys_to_keep
            ]
    ...
    with action_col2:
        if st.button("Back to Execute", width="stretch"):
            st.session_state["step"] = 3
            st.rerun()
